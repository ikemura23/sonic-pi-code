...
live_loop :kick, sync: :met do
  ...
  use_sample_defaults amp: 1.1, lpf: 130
...
live_loop :hhc1, sync: :met do
  ...
  with_fx :hpf, cutoff: 100, amp: 0.8 do
    sample :hat_star, amp: 0.5, rate: 1
    sleep 0.25
    sample :hat_bdu, amp: 0.5, rate: 1
    sleep 0.25
  end
end
...
##|   if melody2_play < 1 then stop end
...
live_loop :pico, sync: :met do
  ...
  use_random_seed 198291
  use_synth :pulse
  use_synth_defaults release: 0.2, amp: 0.4, cutoff: 90
  beats = [0.25, 0.375, 0.125, 0.25].shuffle
  
  cd = chord(:g3, :M7, invert: [0,2].tick).shuffle
  
  sleep 4-beats.sum
  
  with_fx :reverb do
    with_fx :echo, mix: 0.3, phase: 0.375, decay: 2 do
      4.times do
        play cd.tick
        sleep  beats.look
      end
...
live_loop :bass_loop, sync: :met do
  ...
  use_synth :dsaw
  use_synth_defaults release: 0, cutoff: 40, amp: 1
